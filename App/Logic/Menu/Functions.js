...
export const buildMenuTree = (menuKey, parent) => {
    ...
    if (menu.items.constructor !== Array) {
        throw new Error(
            'MenuItems property has to be an array containing the menu items'
        );
    }
    ...
    const menuLevel = usedItems.reduce((level, current, index, array) => {
        ...
        const group = menuGroupItems.find(
            (group) => group.key == current.group
        );
        const children = menu.items.filter(
            (item) => item.parent == current.key
        );
